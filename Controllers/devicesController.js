...
exports.addNewDevice = async (req, res) => {
  try {
    ...
    const { classId, refference, brand, type } = req.body;
    const deviceForAdding = new Devices({ classId, refference, type, brand });
...
exports.updateDevice = async (req, res) => {
  try {
    const id = req.params.id;
    const { refference, brand, type, classId } = req.body;
    console.log(req.body);
    if (id) {
      await devicesService.updateDevice(id, refference, brand, type, classId);
